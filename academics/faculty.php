<?php
    ...
    $query = "SELECT f.`user_id`, f.`role_id`, f.`salutation`, f.`f_name`, f.`m_name`, f.`l_name`, f.`email2`, f.`designation`, f.`research_gate_id`, f.`photo`, f.`contact`, f.`qualification`, f.`interest`, f.`website`, f.`join_date`, l.`email`, (SELECT COUNT(*) FROM `publications` p WHERE p.`user_id` = f.`user_id` GROUP BY(p.`user_id`)) AS `publications`, (SELECT COUNT(*) FROM `books_authored` ba WHERE ba.`user_id` = f.`user_id` GROUP BY(ba.`user_id`)) AS `books_authored` FROM `faculty` f INNER JOIN `login` l ON l.`id` = f.`user_id` WHERE f.`dept_code` = " . $dept_code . " ORDER BY f.`role_id` DESC, f.`f_name`";
    $result = mysqli_query($conn, $query);
    if(!$result) {
        die($query . mysqli_error($conn));
    }

    $query = "SELECT ba.`user_id`, ba.`books` FROM `books_authored` ba INNER JOIN `faculty` f ON f.`user_id` = ba.`user_id` WHERE f.`dept_code` = " . $dept_code . " ORDER BY ba.`user_id`";
...
<html>
    ...
    <body>
        <?= $nav ?>
            <main id="faculty-page">
                <div class="container">
                    <h1 class="position-relative">
                        <span class="b-b-theme-thick"><?= $dept_name ?> - Faculty</span>
                        ...
                        <div class="col-12 col-md-9">
                            <?php
                                while($row = mysqli_fetch_assoc($result)) {
                                    $email = $row['email'];
                                    $name = $row['salutation'] . " " . $row['f_name'] . " " . $row['m_name'] . " " . $row['l_name'];
                                    if($row['email2'] != "") {
                                        $email .= ", " . $row['email2'];
                                    }
                                    $row_span = 8;
                            ...
                            <table class="fac-indv">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td  class="w-60">
                                            <a href="<?= __ROOT__ ?>/academics/department/<?= $dept_url ?>/faculty/<?= $row['user_id'] ?>/publications" target="_blank">Click Here</a>
        ...
        <script>
            $(document).ready(function() {
                $("#nav-academics, .sub-nav-faculty").addClass("active");
            });
        </script>
    </body>
</html>
